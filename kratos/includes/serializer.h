...
//    read(*pValue);
    ...
    template<class TDataType, std::size_t TDataSize>
    void load(std::string const & rTag, boost::array<TDataType, TDataSize>& rObject)
    {
        load_trace_point(rTag);
        for (SizeType i = 0; i < TDataSize; i++)
            load("E", rObject[i]);
    ...
    template<class TDataType, std::size_t TDataSize>
    void save(std::string const & rTag, boost::array<TDataType, TDataSize> const& rObject)
    {
        save_trace_point(rTag);
        for (SizeType i = 0; i < TDataSize; i++)
            save("E", rObject[i]);
    ...
    template<class TDataType>
    void save(std::string const & rTag, boost::intrusive_ptr<TDataType> pValue)
    {
        save(rTag, pValue.get());
    }
...
//    write(*pValue);
    ...
    KRATOS_SERIALIZATION_DIRECT_SAVE(char)
...
//        }
    ...
    template<class TDataType>
    void read(typename MatrixVectorTypeSelector<TDataType>::SymmetricMatrixType& rData)
    {
        read_matrix(rData);
    }
    ...
    template<class TDataType>
    void write(typename MatrixVectorTypeSelector<TDataType>::SymmetricMatrixType const& rData)
    {
        write_matrix(rData);
    }
